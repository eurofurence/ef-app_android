/**
 * Eurofurence API for Mobile Apps
 * No description provided (generated by Swagger Codegen https://github.com/swagger-api/swagger-codegen)
 *
 * OpenAPI spec version: v2
 * 
 *
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen.git
 * Do not edit the class manually.
 */

package io.swagger.client.model;

import java.util.Date;
import java.util.UUID;
import io.swagger.annotations.*;
import com.google.gson.annotations.SerializedName;

@ApiModel(description = "")
public class EventFeedbackRecord {
  
  @SerializedName("LastChangeDateTimeUtc")
  private Date lastChangeDateTimeUtc = null;
  @SerializedName("Id")
  private UUID id = null;
  @SerializedName("EventId")
  private UUID eventId = null;
  @SerializedName("AuthorUid")
  private String authorUid = null;
  @SerializedName("Rating")
  private Integer rating = null;
  @SerializedName("ForwardToPanelist")
  private Boolean forwardToPanelist = null;
  @SerializedName("Message")
  private String message = null;

  /**
   **/
  @ApiModelProperty(required = true, value = "")
  public Date getLastChangeDateTimeUtc() {
    return lastChangeDateTimeUtc;
  }
  public void setLastChangeDateTimeUtc(Date lastChangeDateTimeUtc) {
    this.lastChangeDateTimeUtc = lastChangeDateTimeUtc;
  }

  /**
   **/
  @ApiModelProperty(required = true, value = "")
  public UUID getId() {
    return id;
  }
  public void setId(UUID id) {
    this.id = id;
  }

  /**
   **/
  @ApiModelProperty(value = "")
  public UUID getEventId() {
    return eventId;
  }
  public void setEventId(UUID eventId) {
    this.eventId = eventId;
  }

  /**
   **/
  @ApiModelProperty(value = "")
  public String getAuthorUid() {
    return authorUid;
  }
  public void setAuthorUid(String authorUid) {
    this.authorUid = authorUid;
  }

  /**
   **/
  @ApiModelProperty(value = "")
  public Integer getRating() {
    return rating;
  }
  public void setRating(Integer rating) {
    this.rating = rating;
  }

  /**
   **/
  @ApiModelProperty(value = "")
  public Boolean getForwardToPanelist() {
    return forwardToPanelist;
  }
  public void setForwardToPanelist(Boolean forwardToPanelist) {
    this.forwardToPanelist = forwardToPanelist;
  }

  /**
   **/
  @ApiModelProperty(value = "")
  public String getMessage() {
    return message;
  }
  public void setMessage(String message) {
    this.message = message;
  }


  @Override
  public boolean equals(Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    EventFeedbackRecord eventFeedbackRecord = (EventFeedbackRecord) o;
    return (this.lastChangeDateTimeUtc == null ? eventFeedbackRecord.lastChangeDateTimeUtc == null : this.lastChangeDateTimeUtc.equals(eventFeedbackRecord.lastChangeDateTimeUtc)) &&
        (this.id == null ? eventFeedbackRecord.id == null : this.id.equals(eventFeedbackRecord.id)) &&
        (this.eventId == null ? eventFeedbackRecord.eventId == null : this.eventId.equals(eventFeedbackRecord.eventId)) &&
        (this.authorUid == null ? eventFeedbackRecord.authorUid == null : this.authorUid.equals(eventFeedbackRecord.authorUid)) &&
        (this.rating == null ? eventFeedbackRecord.rating == null : this.rating.equals(eventFeedbackRecord.rating)) &&
        (this.forwardToPanelist == null ? eventFeedbackRecord.forwardToPanelist == null : this.forwardToPanelist.equals(eventFeedbackRecord.forwardToPanelist)) &&
        (this.message == null ? eventFeedbackRecord.message == null : this.message.equals(eventFeedbackRecord.message));
  }

  @Override
  public int hashCode() {
    int result = 17;
    result = 31 * result + (this.lastChangeDateTimeUtc == null ? 0: this.lastChangeDateTimeUtc.hashCode());
    result = 31 * result + (this.id == null ? 0: this.id.hashCode());
    result = 31 * result + (this.eventId == null ? 0: this.eventId.hashCode());
    result = 31 * result + (this.authorUid == null ? 0: this.authorUid.hashCode());
    result = 31 * result + (this.rating == null ? 0: this.rating.hashCode());
    result = 31 * result + (this.forwardToPanelist == null ? 0: this.forwardToPanelist.hashCode());
    result = 31 * result + (this.message == null ? 0: this.message.hashCode());
    return result;
  }

  @Override
  public String toString()  {
    StringBuilder sb = new StringBuilder();
    sb.append("class EventFeedbackRecord {\n");
    
    sb.append("  lastChangeDateTimeUtc: ").append(lastChangeDateTimeUtc).append("\n");
    sb.append("  id: ").append(id).append("\n");
    sb.append("  eventId: ").append(eventId).append("\n");
    sb.append("  authorUid: ").append(authorUid).append("\n");
    sb.append("  rating: ").append(rating).append("\n");
    sb.append("  forwardToPanelist: ").append(forwardToPanelist).append("\n");
    sb.append("  message: ").append(message).append("\n");
    sb.append("}\n");
    return sb.toString();
  }
}
